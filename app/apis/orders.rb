class Orders < Grape::API
  ...
  resource 'orders' do
    ...
    # curl -i -H "Accept: application/json" -X POST -d '{"authentication_token":"CXTTTTED2ASDBSD4", "time_to_ready": "15"}' http://107.22.211.58:9000/api/v1/orders/1/in_progress -v 
    ...
    post "/:id/in_progress" do 
      ...
      logger.info "Authenticated User: #{current_user.full_name}"
      order = Order.find(params[:id])
      order.send_in_progress_nofitication unless order.device_type == 'web'
    ...
    post "/:id/ready" do 
      ...
      logger.info "Authenticated User: #{current_user.full_name}"
      order = Order.find(params[:id])
      order.send_ready_nofitication unless order.device_type == 'web'
